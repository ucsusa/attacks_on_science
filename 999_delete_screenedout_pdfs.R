##This script deletes saved pdfs of articles after 2 months.

DRY_RUN <- as.logical(Sys.getenv("AOS_PDFCLEAN_DRYRUN", "TRUE"))
...
pdf_df_c_date <- map_dfr(pdf_files, function(i) {
  ...
  mutate(c_date = as.Date(creation_date), 
         date_diff_num = as.numeric(today() - c_date)) %>%
