...
#importing RSS Feed articles that have already been web scraped or have read text from a saved pdf
#already_read_urls <- read_csv("C:/AOS_db/data/05_screened_read_articles_complete.csv") %>%
 # select(title, description, pub_date, URL, source, description_original, url_text) %>% #only include these variables
#  unique() %>% #eliminating duplicates
#  filter(!is.na(url_text)) #eliminating rows where the url text is blank

#compile news articles from past three days whose article text hasn't been scraped
#screened_feed_to_read <- anti_join(unread_screened_feed, already_read_urls)
...
for(i in 1:nrow(unread_screened_feed_need_pdf)){ 
  ...
  Sys.sleep(3)
      b$Page$navigate(paste0("https://archive.ph/", url_test))
